...
# Check if this is a Radarr test event
if [ "$radarr_eventtype" = "Test" ]; then
    log_info "Radarr test event detected. Script is functioning correctly."
    exit 0
fi
...
elif [ ! -z "$radarr_movie_path" ]; then
    input_file="$radarr_movie_path"
    log_info "Using radarr_movie_path: $input_file"
else
    log_error "Error: No input file provided. Either pass as argument or set radarr_movie_path environment variable."
    exit 1
fi
